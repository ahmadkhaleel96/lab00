...
function displayPics(){
  ...
  // when changing var to const, the console throw error because the rando is undefined for it because the const is a block hoisted element, so the rando is only defined only on it`s block. as for the const the const is a fixed value the variable is unchangeable, we cant update any const value. it stopped the for loops because the counter is fixed and can't be changed.
